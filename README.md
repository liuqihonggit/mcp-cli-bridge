...
| 约束项          | 规则                                                 | 详细文档                                         |
| ------------ | -------------------------------------------------- | -------------------------------------------- |
| **CLI 工具缓存** | 禁止缓存 CLI 内部工具列表和描述                                 | [AGENTS.md - 核心价值](./AGENTS.md#核心价值)         |
| **构建发布**     | 必须通过 `build.ps1` 脚本发布到 `publish/` 目录               | [AGENTS.md - 构建和发布](./AGENTS.md#构建和发布)       |
| **插件隔离**     | `src/Plugins/` 为外部插件，内部项目不得直接引用                    | [AGENTS.md - CLI 插件系统](./AGENTS.md#cli-插件系统) |
| **接口暴露**     | MCP Server 只暴露 Host 层工具，CLI 工具通过 `tool_execute` 调用 | [CLAUDE.md - 架构约束](./CLAUDE.md#架构约束)         |
...
# 或使用 npx
npx @jingjingbox/mcp-cli-bridge --help
```
...
详细协议说明请查看：[AGENTS.md - CLI 插件系统](./AGENTS.md#cli-插件系统)
...
>
> 详细架构说明请查看：[AGENTS.md - 架构设计](./AGENTS.md#架构设计)
...
详细生命周期和安全机制请查看：[AGENTS.md - CLI 插件系统](./AGENTS.md#cli-插件系统)
...
>
> 详细构建流程、文件规则和错误处理请查看：[AGENTS.md - 构建和发布](./AGENTS.md#构建和发布)
...
| 问题现象           | 可能原因           | 解决方案                | 参考文档                                         |
| -------------- | -------------- | ------------------- | -------------------------------------------- |
| CLI 插件加载失败     | 引用了 McpHost 项目 | 移除对 McpHost 的引用     | [AGENTS.md - 意外情况处理](./AGENTS.md#意外情况处理)     |
| 进程池耗尽          | 超时配置不当或异常未处理   | 检查超时配置和异常处理         | [AGENTS.md - 意外情况处理](./AGENTS.md#意外情况处理)     |
| npm 发布失败版本已存在  | 版本号未递增         | 更新 package.json 版本号 | [AGENTS.md - npm 发布流程](./AGENTS.md#npm-发布流程) |
