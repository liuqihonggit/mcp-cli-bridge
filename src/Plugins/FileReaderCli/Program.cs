...
using var logger = new Logger(
    LogOutput.StdErr,
    LogLevel.Info,
    nameof(FileReaderCli));

var rootCommand = new RootCommand("File Reader CLI Tool - Read file contents efficiently");
...
var commandOption = new Option<string>(
    ...
    description: "Command to execute: read_head, read_tail, list_tools")
{
    IsRequired = false
};
...
rootCommand.SetHandler(async (string jsonInput) =>
{
    ...
    try
    {
        ...
        var request = JsonSerializer.Deserialize(json, CommonJsonContext.Default.FileReaderRequest);
